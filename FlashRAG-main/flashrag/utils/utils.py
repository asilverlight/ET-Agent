...
def get_dataset(config):
    ...
    for split in all_split:
        ...
        if split in ["test", "val", "dev", "processed_test_datas"]:
...
@retry(max=5, sleep=1)
def search(query, top_n=8, return_score=False):
    # print('=============')
    # print('search begin')
    # url = 'http://0.0.0.0:1557/search'
    url = 'http://0.0.0.0:1243/search'
    if query == '':
        return ''
    data = {'query': query, 'top_n': top_n, 'return_score': return_score}
    ...
    # 去除首尾空白字符
    retrieval_texts = response.json()# 返回一个list
    return retrieval_texts

def find_covering_segments(A, segments, B):
        # 找到 B 在 A 中的位置
        start = A.find(B)  # B 是 A 的子串，保证存在
        end = start + len(B) - 1
        
        # 计算每个子段的起始位置
        p = [0]
        for seg in segments:
            p.append(p[-1] + len(seg))
        
        # 找到覆盖 start 和 end 的最小 i 和 j
        i = bisect.bisect_right(p, start) - 1
        j = bisect.bisect_right(p, end) - 1
