...
class Evaluator:
    ...
    def evaluate(self, data):
        ...
        processed_pred = []
        for p in pred:
            # 检查是否包含"So the option is:"或类似格式
            option_match = re.search(r"So the option is:?\s*([A-Za-z0-9]+)", p)
            if option_match:
                # 提取选项部分
                extracted_option = option_match.group(1).strip()
                processed_pred.append(extracted_option)
            else:
                processed_pred.append(p)
        
        # 更新数据集中的预测结果
        if processed_pred != pred:
            data.update_output("pred", processed_pred)
            pred = processed_pred
        # print(data[0])
        # assert False
        result_dict = {}
