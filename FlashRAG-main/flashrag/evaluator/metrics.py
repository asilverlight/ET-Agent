...
class BaseMetric:
    ...
    def get_dataset_answer(self, data):
        if any(choice == [] for choice in data.choices):
            ...
            golden_answers_list = golden_choice_idx_list
            # print('============ all_choices_list', all_choices_list)
            # print('============ golden_choice_idx_list', golden_choice_idx_list)
            # golden_answers_list = [
            ...
            # for choices, choice_idx in zip(all_choices_list, golden_choice_idx_list):
            #     if isinstance(choice_idx, str) and choice_idx in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
            #         # 将A,B,C,D,E转换为0,1,2,3,4的索引
            #         idx = ord(choice_idx) - ord('A')
            #         if 0 <= idx < len(choices):
            #             golden_answers_list.append(choices[idx])
            #         else:
            #             # 处理索引超出范围的情况
            #             golden_answers_list.append("")
            #     else:
            #         # 如果不是预期的选项格式，添加空字符串
            #         golden_answers_list.append("")
