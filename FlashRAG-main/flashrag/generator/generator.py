        ...
        **params,
        ...
        if return_scores:
            ...
            for output in outputs:
                ...
                token_logprobs = [list(score_dict.values())[0].logprob for score_dict in logprobs]
                if self.config['cal_entropy']:
                    total_logprob = sum(token_logprobs)
                    scores.append(-total_logprob)  # 取负值以便计算信息熵
                else:
                    scores.append(token_logprobs)
    ...
    def generate(
        ...
        **params,
        ...
        for idx in range(0, len(input_list), batch_size):
            # 只有当input_list长度大于batch_size时才显示进度条
            if len(input_list) > batch_size:
                iterator = trange(idx, min(idx + batch_size, len(input_list)), desc="Generation process: ")
            else:
                iterator = range(idx, min(idx + batch_size, len(input_list)))
