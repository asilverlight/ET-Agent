    ...
    def __init__(self, config, system_prompt="", user_prompt="", reference_template=None, enable_chat=True):
        ...
        if self.config['dataset_name'] == 'arc':
            self.base_system_prompt = (
                "### Instruction: Select the correct answer from the options provided based on the given document to answer the question. "
                "Only provide a letter of the correct option (e.g., A, B, C, D) and do not output any other words. "
                "\nThe following are given documents.\n\n{reference}"
            )
            self.base_user_prompt = "## Input: \nQuestion: {question}\nOptions: {options}"
        if self.config['dataset_name'] == 'wikiasp':
            self.base_system_prompt = (
                "Write a detailed and coherent summary to the given entity based on the provided documents. "
                "Your response should be a well-structured paragraph that captures the key details and essence of the entity. "
                "Only give me the summary and do not output any other words. "
                "The following are given documents.\n\n{reference}"
            )
            self.base_user_prompt = "Entity: {question}"
        ...
        #     if self.config['save_note'] != 'self-rag':
        #         self.system_prompt += "\nThe following are given documents.\n\n{reference}"
