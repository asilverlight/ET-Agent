...
class Config:
    ...
    def _prepare_dir(self):
        ...
        # print(save_dir)
        # print(self.final_config['generator_model'])
        ...
        self.final_config["save_dir"] = os.path.join(
            save_dir,
            f"{self.final_config['dataset_name']}_{self.final_config['generator_model']}_{save_note}",#current_time.strftime('%Y_%m_%d_%H_%M')
        )
        # print(self.final_config["save_dir"])
        os.makedirs(self.final_config["save_dir"], exist_ok=True)
        # 这行代码的作用是创建保存目录。如果目录已经存在，不会报错（exist_ok=True）。
        # 它确保了配置文件和其他输出可以被正确地写入到指定的保存目录中。
