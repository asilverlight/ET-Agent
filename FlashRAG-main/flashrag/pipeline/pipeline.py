...
from flashrag.utils import get_retriever, get_generator, get_refiner, get_judger, search
...
class SequentialPipeline(BasicPipeline):
    ...
    def naive_run(self, dataset, do_eval=True, pred_process_fun=None):
        ...
        if self.config['dataset_name'] == 'arc':
            ...
            input_prompts = [self.prompt_template.get_string(question=q, options=options) for q in dataset.question]
        else:
            input_prompts = [self.prompt_template.get_string(question=q) for q in dataset.question]
        dataset.update_output("prompt", input_prompts)

        if self.config['return_prob']:
            pred_answer_list, scores = self.generator.generate(input_prompts, return_scores=True)
    ...
    def run(self, dataset, do_eval=True, pred_process_fun=None):
        ...
        # print(dataset.retrieval_results)
        # print(dataset)
        if self.config['dataset_name'] == 'arc':
            options = [' '.join([f"{chr(65+i)}: {opt}" for i, opt in enumerate(choice)]) for choice in dataset.choices]
            dataset.update_output("options", options)
        ...
        if self.refiner:
            ...
            if "llmlingua" in self.refiner.name and input_prompt_flag:
                ...
                if self.config['dataset_name'] == 'arc':
                    ...
                    input_prompts = [
                        self.prompt_template.get_string(question=q, options=options, retrieval_result=r)
                        for q, r in zip(dataset.question, dataset.retrieval_results)
                    ]
                else:
                    input_prompts = [
                ...
                if self.config['dataset_name'] == 'arc':
                    options = dataset.options
                    input_prompts = [
                        self.prompt_template.get_string(question=q, options=options, formatted_reference=r)
        ...
        #     pred_answer_list = self.generator.generate(input_prompts)
        #     dataset.update_output("pred", pred_answer_list)
        llmlingua_outputs = dataset.refine_result

        # dataset = self.evaluate(dataset, do_eval=do_eval, pred_process_fun=pred_process_fun)
        # if self.config['return_prob']:
        #     return dataset, scores
        dataset.update_output("llmlingua_outputs", llmlingua_outputs)
        return dataset
...
class AdaptivePipeline(BasicPipeline):
    def __init__(
        ...
        mysystem_prompt = "Answer the question based on your own knowledge. Only give me the answer and do not output any other words."
        myuser_prompt = "Question: {question}"
        if self.config['dataset_name'] == 'arc':
            mysystem_prompt = "Answer the question based on your own knowledge. Only give me the correct option (A, B, C, D) and do not output any other words."
            myuser_prompt = "Question: {question}\nOptions: {options}"
        elif self.config['dataset_name'] == 'triviaqa':
            mysystem_prompt = "Generate a summary for the entity based on your own knowledge. Only give me the summary and do not output any other words."
            myuser_prompt = "Entity: {question}"
            

        if norag_template is None:
            norag_templete = PromptTemplate(
                config=config,
                system_prompt=mysystem_prompt,
                user_prompt=myuser_prompt,
            )
        ...
        self.multi_hop_pipeline = IRCOTPipeline(
            config, prompt_template=multi_hop_prompt_template, retriever=retriever, generator=generator, max_iter=4
        )
