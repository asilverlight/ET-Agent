...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    """
    # try:
    #     print('=============================== non tensor batch (JSON): ===============================')
    #     print(json.dumps(batch.non_tensor_batch['raw_score'], ensure_ascii=False, indent=4))
    #     print('=============================== end non tensor batch (JSON): ===========================')
    # except Exception as e:
    #     print('非张量批次转为JSON失败，回退为repr:', str(e))
    #     print(repr(batch.non_tensor_batch))
    # assert False

    # 计算每条样本的工具调用次数与原始正确性分数（不含任何惩罚），进而计算efficiency
    # 1) 工具调用次数优先从reward_extra_info携带的tool_call取值，否则基于响应字符串统计<tag>
    response_strs = batch.non_tensor_batch.get("responses_str", None)
    tool_calls_from_reward = batch.non_tensor_batch.get("tool_call", None)
    if tool_calls_from_reward is not None:
        tool_use_counts = [int(x) for x in tool_calls_from_reward]
    elif response_strs is not None:
        tool_use_counts = [count_valid_tags(s, 'search') + count_valid_tags(s, 'python') for s in response_strs]
    else:
        tool_use_counts = None

    # 2) 原始正确性分数（math: is_equiv的0/1；qa: f1）优先从reward_extra_info携带的raw_score取值
    raw_scores = batch.non_tensor_batch.get("raw_score", None)

    # 3) 计算效率：raw_score / max(tool_calls, 1)
    if raw_scores is not None and tool_use_counts is not None:
        device = batch.batch["responses"].device
        raw_scores_tensor = torch.tensor(raw_scores, dtype=torch.float32, device=device)
        tool_calls_tensor = torch.tensor(tool_use_counts, dtype=torch.float32, device=device)
        efficiency = raw_scores_tensor / torch.clamp(tool_calls_tensor, min=1.0)
        efficiency_metrics = {
            "efficiency/mean": torch.mean(efficiency).detach().item(),
            "efficiency/max": torch.max(efficiency).detach().item(),
            "efficiency/min": torch.min(efficiency).detach().item(),
            "efficiency/tool_calls/mean": torch.mean(tool_calls_tensor).detach().item(),
            "efficiency/tool_calls/max": torch.max(tool_calls_tensor).detach().item(),
            "efficiency/tool_calls/min": torch.min(tool_calls_tensor).detach().item(),
        }
    else:
        efficiency_metrics = {}
    ...
    # 合并efficiency统计（若可用）
    base_metrics = efficiency_metrics.copy()
