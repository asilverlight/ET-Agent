...
async def search(request: QueryRequest):
    ...
    async with retriever_semaphore:
        ...
        try:
            ...
            if return_score:
                return [Document(id=str(result['id']), contents=result['contents']) for result in results], scores
            else:
                return [Document(id=str(result['id']), contents=result['contents']) for result in results]
        finally:
    ...
    async with retriever_semaphore:
        ...
        try:
            if return_score:
                ...
                return [[Document(id=str(result['id']), contents=result['contents']) for result in results[i]] for i in range(len(results))], scores
            else:
                results = retriever_list[retriever_idx].batch_search(query, top_n, return_score)
                return [[Document(id=str(result['id']), contents=result['contents']) for result in results[i]] for i in range(len(results))]
        finally:
            available_retrievers.append(retriever_idx)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", 
        type=str, 
        default="/ET-Agent/retriever/serving_config.yaml",
