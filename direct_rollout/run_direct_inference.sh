...
# 这是训好的推理模型
ENDPOINTS_INFER=$(echo "${infer_endpoints[@]}" | tr '\n' ' ')
ENDPOINTS_SUMMARY=$(echo "${summary_endpoints[@]}" | tr '\n' ' ')

SAMPLE_TIMEOUT=1200  # Timeout for one sample

MODEL_PATH_INFER="/path/to/your_model_path"
MODEL_PATH_SUMMARY="/path/to/qwen2.5-72b-instruct"
OUTPUT_PATH="/path/to/output_direct_rollout.json"                       
DATA_PATH="/path/to/data.json"  

with_tools=true
if [ "$with_tools" = true ]; then
    PROMPT_TYPE="toolstar"          # Prompt type (code_search, search, math, base)
...
DEFAULT_MODEL_INFER="YOUR_MODEL_NAME"  # Default model name
DEFAULT_MODEL_SUMMARY="Qwen2.5-72B-Instruct"
...
COUNTS=20000                        # Number of samples to process
...
SEARCH_MAX_RESULTS=4                            # Max number of search results
SEARCH_RESULT_LENGTH=1200                        # Max length per search result
...
# SUMM_MODEL_URLS="http://localhost:8003/v1"
SUMM_MODEL_URLS=$ENDPOINTS_SUMMARY
SUMM_MODEL_NAME=$DEFAULT_MODEL_SUMMARY
SUMM_MODEL_PATH=$MODEL_PATH_SUMMARY
# 这是总结摘要模型
...
CMD="python -u run.py"
CMD+=" --endpoints_infer $ENDPOINTS_INFER"
CMD+=" --endpoints_summary $ENDPOINTS_SUMMARY"
CMD+=" --model_path_infer $MODEL_PATH_INFER"
CMD+=" --model_path_summary $MODEL_PATH_SUMMARY"
CMD+=" --default_model_infer $DEFAULT_MODEL_INFER"
CMD+=" --default_model_summary $DEFAULT_MODEL_SUMMARY"
