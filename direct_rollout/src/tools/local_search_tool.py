...
class LocalSearchTool(BaseTool):
    ...
    async def postprocess_search_result(self, query, response, **kwargs) -> str:
        ...
        reasoning_path = kwargs["existing_logs"]
        search_query = query
        document = response
    ...
    async def search(self, query: str) -> str:
        ...
        if response.status_code == 200:
            ...
            for line in response.json():
                curr_result += f"{line['contents'][:self._result_length]}...\n\n" if len(line['contents']) > self._result_length else f"{line['contents']}\n\n"
