...
class LocalSearchTool(BaseTool):
    ...
    async def search(self, query: str) -> str:
        ...
        if response.status_code == 200:
            ...
            for line in response.json():
                ...
                retrieval_text += f"{line['contents']}\n\n" if len(line['contents']) < self._result_length else f"{line['contents'][:self._result_length]}...\n\n"
            # 去除首尾空白字符
    ...
    async def execute(self, query: str, timeout: int = 60, **kwargs) -> str:
        ...
        # print(self._use_summarize)
        if self._use_summarize and self._summarize_tool is not None:
