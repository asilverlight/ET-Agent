...
class Evaluator:
    ...
    def calculate_overall_metrics(self, data: List[Dict[str, Any]]) -> Dict[str, Union[float, str]]:
        ...
        """
        num_valid_answer = sum(item.get('metrics', {}).get('is_valid_answer', False) for item in data)
        avg_em = [item.get('metrics', {}).get('em', 0.0) for item in data]
        avg_acc = [item.get('metrics', {}).get('acc', 0.0) for item in data]
        avg_f1 = [item.get('metrics', {}).get('f1', 0.0) for item in data]
        avg_math = [item.get('metrics', {}).get('math_equal', 0.0) for item in data]
        avg_llm = [item.get('metrics', {}).get('llm_equal', 0.0) for item in data]
        avg_tool_counts = [item.get('metrics', {}).get('tool_counts', 0.0) for item in data]
        avg_python_calls = [item.get('metrics', {}).get('python_calls', 0.0) for item in data]
        avg_search_calls = [item.get('metrics', {}).get('search_calls', 0.0) for item in data]
        ...
        if self.task_type == 'math':
            ...
            avg_accuracy = avg_llm
        elif self.task_type == 'qa':
            accuracy = np.mean(avg_f1) if avg_f1 else 0.0
            avg_accuracy = avg_f1
        else:
            accuracy = 0.0
            avg_accuracy = [0.0] * len(data)
        ...
        avg_efficiency = [avg_accuracy[i] / avg_tool_counts[i] if avg_tool_counts[i] > 0 else avg_accuracy[i] for i in range(len(data))]
        final_efficiency = np.mean(avg_efficiency) if avg_efficiency else 0.0
