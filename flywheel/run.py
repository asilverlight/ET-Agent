...
def parse_arguments():
    ...
    vllm_group.add_argument("--endpoints_m1", type=str, nargs='+', required=True,
                            help="List of VLLM endpoints")
    vllm_group.add_argument("--endpoints_m2", type=str, nargs='+', required=True,
                            help="List of VLLM endpoints")
    vllm_group.add_argument("--endpoints_m3", type=str, nargs='+', required=True,
                            help="List of VLLM endpoints")
    vllm_group.add_argument("--model_path_m1", type=str, required=True,
                            help="Model path for tokenizer loading")
    vllm_group.add_argument("--model_path_m2", type=str, required=True,
                            help="Model path for tokenizer loading")
    vllm_group.add_argument("--model_path_m3", type=str, required=True,
                            help="Model path for tokenizer loading")
    vllm_group.add_argument("--api_keys", type=str, nargs='+', default=None,
                            help="List of API keys corresponding to endpoints")
    vllm_group.add_argument("--default_model_m1", type=str, default="Qwen2.5-7B-Instruct",
                            help="Default model name to use")
    vllm_group.add_argument("--default_model_m2", type=str, default="Qwen2.5-7B-Instruct",
                            help="Default model name to use")
    vllm_group.add_argument("--default_model_m3", type=str, default="Qwen2.5-7B-Instruct",
                            help="Default model name to use")
    ...
    generation_group.add_argument("--stop", type=str, nargs='+', default=["</python>", "</search>", "</answer>"],
                                  help="Stop strings")
    ...
    inference_group.add_argument("--additional_python_times", type=int, default=5,
                                 help="Additional number of Python tool invocations")
    inference_group.add_argument("--additional_search_times", type=int, default=5,
                                 help="Additional number of search tool invocations")
    ...
    inference_group.add_argument("--use_sds", action='store_true',
                                 help="Whether to use SimpleDeepSearcher")
    inference_group.add_argument("--max_evolve_times", type=int, default=5,
                                 help="Maximum number of evolve trajectories times")
    inference_group.add_argument("--use_log", action='store_true',
    ...
    tools_group.add_argument("--local_search_url", type=str, default="http://localhost:8004/v1",
                             help="Local search URL")
    tools_group.add_argument("--compatible_search", action='store_true',
                             help="Whether to use compatible search")
    tools_group.add_argument("--max_search_result_length", type=int, default=20000,
                             help="Maximum search result length")
    tools_group.add_argument("--max_python_result_length", type=int, default=1000,
                             help="Maximum python result length")
...
def get_inference_instance():
    ...
    print(vars(args))
    from src.inference import AsyncInferenceCompletionSDS as AsyncInfer
